...
public class Main {
    ...
    private Screening initializeData() {
        ...
        discountConditionRepository.save(DiscountCondition.builder().policyId(discountPolicy.getId()).conditionType(SEQUENCE_CONDITION).sequence(1).build());
        discountConditionRepository.save(DiscountCondition.builder().policyId(discountPolicy.getId()).conditionType(SEQUENCE_CONDITION).sequence(10).build());
        discountConditionRepository.save(DiscountCondition.builder().policyId(discountPolicy.getId()).conditionType(PERIOD_CONDITION)
                .dayOfWeek(MONDAY)
                .startTime(of(10, 0))
                .endTime(of(12, 0))
                .build());
